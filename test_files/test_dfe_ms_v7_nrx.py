...
if __name__ == "__main__":
    ...
    snr_db = 5 #np.array([5, 8, 12, 15])
    mse = np.zeros_like(n_array)
    mse_dl = np.zeros_like(mse)
    d_hat_cum = np.zeros((len(mse),Nd-300-1), dtype=complex) # has to change if Nt changes :(
    ...
    for ind in range(len(n_array)):
        ...
        for i in range(K0):
            ...
            v = transmit(v,snr,Fs,fs,fc,n_rx,d0,uf) # this already does rough phase alignment
            vp = v[:len(up)+Nz*Ns]
            delval,_ = fdel(vp,up)
            ...
            if i == 0:
                v_multichannel = np.zeros_like(v)
    ...
    for ind in range(len(n_array)):
        ...
        plt.axis('square')
        plt.axis([-2, 2, -2, 2])
        plt.title(f'M={n_array[ind]}')

    fig, ax = plt.subplots()
    ax.plot(n_array, mse,'o')
    ax.set_xlabel(r'Number of Elements')
    ax.set_ylabel('MSE (dB)')
    ax.set_title(r'MSE vs Number of Elements for QPSK Signal UL, d0 = 0.5 $\lambda$')

    np.save('data/d_hat_cum3.npy', d_hat_cum)
    np.save('data/mse3.npy',mse)
    plt.show()
    print(mse)
