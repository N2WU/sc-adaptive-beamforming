...
if __name__ == "__main__":
    ...
    if downlink:
        ...
        for ind in range(len(N_FF)):
            ...
            dhat = d_hat_dl_cum_wk[ind]
            #plt.scatter(np.real(d_hat_dl_cum[ind,:]), np.imag(d_hat_dl_cum[ind,:]), marker='x',color='blue')
            plt.scatter(np.real(dhat), np.imag(dhat), marker='x',color='orange')
            plt.axis('square')
            plt.axis([-2, 2, -2, 2])
        ...
        for ind in range(len(N_FF)):
            plt.subplot(2, 2, int(ind+1))
            dhat = d_hat_dl_cum[ind]
            #plt.scatter(np.real(d_hat_dl_cum[ind,:]), np.imag(d_hat_dl_cum[ind,:]), marker='x',color='blue')
            plt.scatter(np.real(dhat), np.imag(dhat), marker='x',color='blue')
            plt.axis('square')
            plt.axis([-2, 2, -2, 2])
            plt.title(f'SNR=10 dB, N_FF={N_FF[ind]}') #(f'd0={d_lambda[ind]}'r'$\lambda$')
            #plt.legend(["No Beamform","Beamform DL"])
        fig3.suptitle('DL QPSK Constellation, 2-path, Varying N_FF, No BF')
