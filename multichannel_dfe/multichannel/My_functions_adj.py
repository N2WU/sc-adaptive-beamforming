...
def receive_signal(snr, fc=6.5e3):
    ...
    modem = QAMModem(M)
    preamble = generate_preamble(7, modem)
    ...
    preamble_upsampled = sg.resample_poly(preambles,uf,1)
    ...
    el_spacing = 0.5*343/fc
    n_rx = 12

    reflection_list = np.asarray([1,0.5]) # reflection gains
    x_tx_list = np.array([5,-5]) #([5,-5]) 
    y_tx_list = np.array([20,20]) #([20,20])
    c = 343
    ...
    # line-of-sight, no reflect channel
    # empty_sig = np.concatenate((np.zeros(int(0.25*Fs)), s))
    # empty_sig = np.concatenate((empty_sig, np.zeros(int(0.25*Fs))))
    # r_multichannel = np.tile(empty_sig,(n_rx,1))
    return r_multichannel

def get_S_theta(r_multichannel, fc, n_path=1, theta_start=-45, theta_end=45, plot_S_theta=True, plot3D_S_theta=False):
...
def receiver_processing(r_multichannel, fc, K_list=[1], feedforward_taps=20, feedbackward_taps=3, alpha_rls=0.999):
    # fc = np.load('data/fc.npy')
    Fs = 96000
    ...
    for i in range(K):
        ...
        if i == 0:
            # sample-based xcorr
            xcorr_for_peaks = np.abs(sg.fftconvolve(v_xcorr, sg.resample_poly(preamble[::-1].conj(),uf,1)))
            # symbol-based xcorr (?)
    ...
    for K_channels in K_list:
        d_hat, mse, n_err, n_training = processing(v_rls, d, K_channels)
        plt.figure()
...
if __name__ == "__main__":
    ...
    snr_db = np.array([30]) #np.array([8, 12, 16, 20])
    mse = np.zeros(len(snr_db),dtype='float')
        
    for ind in range(len(snr_db)):
        snr = 10**(0.1 * snr_db[ind])
        r_multichannel = receive_signal(snr, fc)
        y = get_S_theta(r_multichannel, fc, n_path=n_path, theta_start=-45, theta_end=45, 
                plot_S_theta=False, plot3D_S_theta=False)
        d_hat, mse_out = receiver_processing(y, fc,
                    K_list=[n_path], feedforward_taps=20, feedbackward_taps=3, alpha_rls=0.5)
        mse[ind] = mse_out
