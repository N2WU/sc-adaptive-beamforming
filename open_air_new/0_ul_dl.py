...
def uplink(v,snr,Fs,fs,fc,n_rx):
    ...
    s = snr*np.real(vs*np.exp(1j*2*np.pi*fc*np.arange(len(vs))/Fs))
    s_tx = np.copy(s) # this can't work for multiuser
    r_multi = np.zeros((len(s),n_rx))

    r_multi = testbed(s_tx,n_rx) # s-by-nrx
...
def downlink(v_dl,wk,snr,Fs,fs,fc):
    vs = sg.resample_poly(v_dl,Fs,fs)
    s_dls = snr*np.real(vs*np.exp(1j*2*np.pi*fc*np.arange(len(vs))/Fs))
    # apply wk here
    s_dl = np.dot(np.reshape(wk,[-1,1]),np.reshape(s_dls,[1,-1])) #s-by-nrx

    r_single = testbed(np.real(s_dl),1)
...
if __name__ == "__main__":
    ...
    for ind in range(len(snr_db)):
        ...
        v_single = downlink(v_dl,wk,snr,Fs,fs,fc)
